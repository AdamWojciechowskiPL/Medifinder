...
async function toggleBackendScheduler() {
    ...
    if (enabled) {
        ...
        // KRYTYCZNA NAPRAWA: Odznaczenie checkboxa ZATRZYMUJE scheduler
...
async function startBackendScheduler() {
    ...
    try {
        ...
        if (data.success) {
            showToast(`✅ Scheduler uruchomiony (co ${intervalMin} min, auto-stop po 24h)`, 'success');
    ...
    try {
        ...
        if (data.success && data.data) {
            ...
            if (status.active) {
                ...
                // Inactive - SYNCHRONIZUJ CHECKBOX!
                document.getElementById('enableAutoCheck').checked = false;
...
function updateSchedulerDetails(status) {
    ...
    // Auto-stop info
    if (status.stop_time) {
        const stopTime = formatLocalDateTime(status.stop_time);
        html += `<span>⏰ Auto-stop: ${stopTime}</span>`;
    }
